...
from detection.test import test_net

from detection2.demo import demo
...
class det2Option():
    def __init__(self):
        ...
        self.workers=6
        self.batch_size = 192
        self.saved_model = './yolov5/detection2/TPS-ResNet-BiLSTM-Attn.pth'
...
def apply_classifier(x, model, img, im0):
    ...
    im0 = [im0] if isinstance(im0, np.ndarray) else img
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None,index=0): 
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    ocr_img = img[c1[1]:c2[1],c1[0]:c2[0]]
    cv2.imwrite(f"test/catch_{index}.jpg",ocr_img)
    ocr_img = cv2.resize(ocr_img,dsize=(0,0),fx=5.0,fy=5.0,interpolation=cv2.INTER_LINEAR)
    boxes,polys,_ = test_net(net,ocr_img)
    back_no=None
    
    # cv2.imshow('t',img[c1[1]:c2[1],c1[0]:c2[0]])
    # cv2.waitKey(0)
    if len(polys)>0:
        for i,box in enumerate(polys):
            ...
            print(box)
            padding = 0.4
            x1 = 0 if box[0]*(1-padding)<0 else ocr_img.shape[0] if ocr_img.shape[0]<box[0]*(1-padding) else box[0]*(1-padding)
            x2 = 0 if box[2]*(1+padding)<0 else ocr_img.shape[1] if ocr_img.shape[1]<box[0]*(1+padding) else box[2]*(1+padding)
            y1 = 0 if box[1]*(1-padding)<0 else ocr_img.shape[0] if ocr_img.shape[0]<box[0]*(1-padding) else box[1]*(1-padding)
            y2 = 0 if box[5]*(1+padding)<0 else ocr_img.shape[1] if ocr_img.shape[1]<box[0]*(1+padding) else box[5]*(1+padding)
            x1 = int(x1)
            x2 = int(x2)
            y1 = int(y1)
            y2 = int(y2)
            ...
            try:
                cv2.imwrite(f"test/test{index}_{i}.jpg",ocr_img[y1:y2,x1:x2])
                print("------demo------")
    ...
    if label:
        ...
        cv2.putText(img, label if back_no is None else label+str(back_no), (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
